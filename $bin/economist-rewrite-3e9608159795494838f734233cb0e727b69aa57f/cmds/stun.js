...
module.exports = {
	...
	async run(client, message, args) {
		...
		await client.db.set("stn" + usr.id, client.config.parseCd(message.createdTimestamp, Number(args[1])*60000));
		await client.db.set("dns" + usr.id, client.config.parseCd(message.createdTimestamp, Number(args[1])*60000));
