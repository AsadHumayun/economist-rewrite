...
module.exports = {
	...
	async run(client, message, args) {
		if (args.length < 2) return message.reply("You must specify a role<keyword> and member for this command to work!");
		...
		if (kws.includes(kw)) return message.reply("That user already has a role by that keyword.");
		if(!args[2]) args[2] = "";
