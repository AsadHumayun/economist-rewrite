...
module.exports = {
	...
	async run(client, message, args) {
		if (args.length < 2) return message.reply("You must use the correct format; `" + message.guild.prefix + "editreplacer <replacer keyword> <new content>`");
		...
		if (!Object.keys(data).includes(kw)) {
			return message.reply(`A supplanter by that name was not found. Look in \`${message.guild.prefix}replacers\` to view a list and \`${message.guild.prefix}addreplacer <keyword> <content>\` to add a new one.`);
		}
