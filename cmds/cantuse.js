...
module.exports = {
	...
	async run(client, message, args) {
		if(!args.length) return message.reply("You must include a command name/alias in order for this command to work")
	...
	if (!command) return message.reply("A command with that name or alias was not found.");
		const msg = await message.reply(`Fetching Users...`);
		Promise.all(
