...
module.exports = {
	...
	async run(client, message, args) {
		let cst = await client.db.get("cst" + message.author.id) || "";
				cst = cst.split(";")
		const fish_rod = cst.includes("fishrod");
		...
		message.reply({ embed: new MessageEmbed().setDescription(`${message.author.tag} locates their ${client.config.emoji.fishing_rod} and goes fishing...`).setColor(message.author.color) })
		await delay(2000);
		...
		message.reply({ embed: new MessageEmbed().setDescription(`${message.author.tag} sits down near a calm pool of water... :droplet:`).setColor(message.author.color) })
		await delay(2000);
		message.reply({
