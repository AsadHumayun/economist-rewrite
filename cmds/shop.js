...
export default {
	...
	async run(client, message, args) {
		...
		const shop = new MessageEmbed()
			...
			.addField("Items", `[1] ${client.const.emoji.fishing_rod}${owns.fishing_rod ? ` ${client.const.emoji.tick} **ALREADY OWNED** ` : " "}- Allows you to go fishing via \`${message.guild ? message.guild.prefix : client.const.prefix}fish\`; costs :dollar: 25\n[2] ${client.const.emoji.mobile_phone}${owns.phone ? ` ${client.const.emoji.tick} **ALREADY OWNED** ` : " "}- Allows you to comunicate with others via \`${message.guild ? message.guild.prefix : client.const.prefix}dial\`; costs :dollar: 750\n[3] ${client.const.emoji.phonebook}${owns.phonebook ? ` ${client.const.emoji.tick} **ALREADY OWNED** ` : " "}- Allows you to view other users' phone numbers via \`${message.guild ? message.guild.prefix : client.const.prefix}number\`; costs :dollar: 250\n[4] :receipt:${owns.slrprmt ? ` ${client.const.emoji.tick} **ALREADY OWNED** ` : " "}- The Seller's Permit - Allows you to sell in-game items via \`${message.guild ? message.guild.prefix : client.const.prefix}sell\`; costs :dollar: 50,000`)
			.addField("Foodstuffs", `[101] ${client.const.emoji.chill} - 1x Chill Pill - removes **all** cooldowns, 6 hour cooldown for consuimg this item; consume with \`${message.guild ? message.guild.prefix : client.const.prefix}dose chill\`; costs :dollar: 100 each`)
