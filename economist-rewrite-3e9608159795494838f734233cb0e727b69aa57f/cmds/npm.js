...
module.exports = {
	...
	async run(client, message, args) {
		...
		if (res.status == 404) {
			return msg.edit(client.config.statics.defaults.emoji.err + " I could not find the specified package");
		}
		const body = await res.json();
		if (body.time.unpublished) {
			return msg.edit(client.config.statics.defaults.emoji.err + " The specified package is unpublished");
		}
		...
		const embed = new MessageEmbed()
			...
			.addField("❯ Dependencies", dependencies.length > 0 ? dependencies.join(", ") : "None")
