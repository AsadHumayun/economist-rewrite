	...
	async run(client, message, args) {
		async function Embed(u) {
			...
			if (!cst.includes("dragon") || (!data)) {
				return message.reply(`${message.author.id == u.id ? "You don't own a dragon!" : `${u.tag} does not own a dragon!`} Why not tame one by using \`${message.guild ? message.guild.prefix : client.const.prefix}tame\``);
			}
			...
			// alias[0] is the alias.displayname - refer to ../config.js: Funcs.getDragonAlias (async function)
				.setDescription(`\`${message.guild ? message.guild.prefix : client.const.prefix}disown\` to disown your ${alias[0]} and delete it.\n\`${message.guild ? message.guild.prefix : client.const.prefix}feed\` to feed your ${alias[0]} and completely refill its energy.\n\`${message.guild ? message.guild.prefix : client.const.prefix}name <new name>\` to name your ${alias[0]}. (requires Supreme)\n\`${message.guild ? message.guild.prefix : client.const.prefix}stroke\` to stroke your ${alias[0]} and increase its affection by 1.\n\`${message.guild ? message.guild.prefix : client.const.prefix}search\` to get your ${alias[0]} to go out searching for fish and gain a certain amount of XP depending on your ${alias[0]}'s intellect.\n\`${message.guild ? message.guild.prefix : client.const.prefix}upgrade <stat> <amount>\` to upgrade \`<stat>\` by \`<amount>\` points, \`<amount>\` defaults to 1.\n\`${message.guild ? message.guild.prefix : client.const.prefix}downgrade <stat>\` to downgrade a stat by 1 point and receive ${alias[1][3]} 1 in return \`${message.guild ? message.guild.prefix : client.const.prefix}deprive <stat>\` to completely remove all invested ${alias[1][3]} on the stat and receive the appropriate amount of credits in return. For example, say a user has 5 total points on strength: \`${message.guild ? message.guild.prefix : client.const.prefix}deprive strength\` would set the strength stat to 1 and return 4 credits to the user.\n\`${message.guild ? message.guild.prefix : client.const.prefix}defend\` to toggle your ${alias[0]}'s protection     —     your ${alias[0]} must have at least 200 health in order for it successively to protect you from attackers.`)
				.addField(
					"Basic Stats",
					`
