...
export default {
	...
	async run(client, message, args) {
		...
		if (!u1) return message.reply({ content: `Invalid argument for user1: "${args[0]}"`, allowedMentions: { parse: [] } });
		const u2 = await client.utils.fetchUser(args[1]);
		if (!u2) return message.reply({ content: `Invalid argument for user2: "${args[1]}"`, allowedMentions: { parse: [] } });
