...
export default {
	...
	async run(client, message, args) {
		...
		if (!isNaN(args[1])) {
			client.utils.updateBalance(usr, BigInt(0 - Number(args[1])), message);
			return message.reply({
				embeds: [
					new MessageEmbed()
						.setColor(message.author.color)
						.setDescription(`:dollar: ${client.utils.comma(BigInt(args[1]))} have been removed from ${usr.tag}'s account`),
				],
			});
		}
