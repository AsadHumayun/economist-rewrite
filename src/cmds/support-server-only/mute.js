...
export default {
	...
	async run(client, message, args) {
		...
		if (member.roles.cache.has(client.const.roles.mod.trial)) return message.reply(`${client.const.emoji.err} You're not allowed to mute a moderator!`);
		await member.roles.add(client.const.roles.muted);
		const amt = Number(args[1]);
		if (isNaN(amt) || (!args[1])) return message.reply(`${client.const.emoji.err} You must provide a valid length (in minutes). For permanent mutes, use 0 as the length.`);
		...
		message.reply({
			embeds: [
				new MessageEmbed()
					...
					.setDescription(`${client.const.emoji.tick} ${member.user.tag} was given a ${amt == 0 ? "permanent" : `${amt} minute`} mute because of "${reason}"; they have been sent the following message:`),
			],
		});
		...
		setTimeout(async () => {
			if (!member.roles.cache.has(client.const.roles.muted)) return;
			member.roles.remove(client.const.roles.muted);
			member.send({
				embeds: [
					new MessageEmbed()
			...
			client.channels.cache.get(client.const.channels.modlog).send(`${Date.now() / 60_000}: (${message.guild.name} [${message.guild.id}])[${message.channel.name}]<${client.user.tag} (${client.user.id})>: unmute ${member.id} R: mute time of ${amt} minutes has passed`);
		}, amt * 60_000);
	},
};