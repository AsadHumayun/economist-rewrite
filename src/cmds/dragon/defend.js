...
export default {
	...
	async run(client, message) {
		...
		const p = message.author.data.get("drgn").split(";").map(BigInt);
		if (p[1] < BigInt(200)) return message.reply("Your " + dragonAlias[0] + " must have at least " + dragonAlias[1][0] + " 200 in order to defend you from attackers.");
