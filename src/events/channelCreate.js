export default {
	...
	async execute(client, channel) {
		if (["DM", "GROUP_DM"].includes(channel.type) || (channel.guild.id != client.const.supportServer)) return;
	...
	Can manage: [false] (member: ${usr.id}, channel: ${channel.id}, allow: ${x.allow.bitfield}, deny: ${x.deny.bitfield})
			...
			client.channels.cache.get(client.const.channels.sflp).send({ content: `${Math.trunc(Date.now() / 60000)} U:<${usr.tag} (${usr.id})>: Successfully set chnl as ${client.utils.trim(chn.join(";"), 1900)} ` });
		});
	},
};